...
import DetailDiscount from "components/card/DetailDiscount";
  ...
  return (
    <div className="bg-white w-full h-full rounded-md shadow-sm shadow-black">
      <div className="bg-red-500 h-12 w-full flex items-center justify-between px-3 rounded-t-md">
        <p className="text-white font-bold">Detail Discount</p>
        <button
      ...
      {useMemo(() => {
        return <DetailDiscount globalCtx={globalCtx} globalAct={globalAct} />;
      }, [globalCtx.selectedData])}
    </div>
  );
};

export default memo(DiscountDetailModal);
