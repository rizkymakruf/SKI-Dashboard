...
import FormSubCategoryEdit from "components/form/FormSubCategoryEdit";
  ...
  return (
    <div className="bg-white w-full h-full rounded-md shadow-sm shadow-black">
      <div className="bg-red-500 h-12 w-full flex items-center justify-between px-3 rounded-t-md">
        <p className="text-white font-bold">Update Sub Kategori</p>
        <button
          onClick={() => {
            globalAct.setModal(""),
      ...
      {useMemo(() => {
        return <FormSubCategoryEdit />;
      }, [globalCtx.modal])}
    </div>
  );
};

export default memo(SubCategoryUpdate);
